...
%% Altered from a tex.stackoverflow.com answer to make smaller numbers
%% in smaller circles
...
brutalize it.  Wood is not only of the tree: it is a spar, a rib. Look
at it closely; see if it is healthy.
...
reason to open their big mouths. Amateurs will soon be able to do more
than fly around the pattern, and Mignet the industrialist will not
have to completely shed his amateur's skin\footnote{The meaning of
  this passage, \textit{\guillemotleft Les amateurs pourront bient\^ot
    s'\'evader des tours de piste et Mignet l'industriel n'aura pas
...
\textit{(fig. \ref{fig:twenty})} of hardwood --- walnut, oak, beech,
etc. --- $20\times26\times230$.  Only drill one side of these fittings
for now.  The other side will be drilled later when it is bound or
clamped in place.
...
\circled{47} \textit{(fig. \ref{fig:twentyfour})}, cut so that its
cross member \circled{48} fits on the shim \circled{35}, and its
crosspiece \circled{49} at the bottom of the tabs \circled{33} so that
cross members and flanges are flush with the longerons \circled{12}.
The height of the panel \circled{47} will be determined in place.  Do
not forget that the hole \circled{50} is reinforced with a ring.  File
...
\textit{(fig. \ref{fig:eighteen})}.  You will have to use a brace and
bit to drill a hole \circled{164} as seen at \circled{165}, and use a
rasp to fit it to the diameter of the tube, \textit{i.e.} 24mm.
...
\textit{(fig. \ref{fig:thirtyfive})} where it is attached by a cable
...
The rounded leading edge is cut from a 12mm-thick board.
...
strip are then placed on edge \circled{230}\footnote{This refers to
...
\section{Propeller shaft}

At what speed does this used engine you found normally run?  Would it
work to just stick a propeller on it, direct-drive?  Has the pull of a
propeller been planned on its bearings?  Of course not.

It's much easier to use the propeller made by the book by powering it
like a motorcycle wheel, using a reduction drive whose ratio will be
determined empirically, without risking damaging the engine.
...
A threaded rod \circled{197}, 200mm long, secures the U-fitting
\circled{198}/\circled{199} under the middle of the front spar of the
rear wing.  The fittings \textbf{B} fixed by the two bolts
\circled{365} have been cut and bent so that their eyes, by which the
stay cables are attached, are aligned with the axis of the pivot
attachment \circled{195}, as shown in exaggerated form at
\circled{368}.  With a tautened wire, it is easy to determine the
height \circled{369} \textit{(fig. \ref{fig:eightyone})}.

\begin{figure}
  \includegraphics[width=\linewidth]{fig-81.jpg}
  \caption{Layout of front-wing bracing}
  \label{fig:eightyone}
\end{figure}

The axis of rotation \circled{195} for the rear wing is held between
two brackets \circled{188}, which are fixed to cross-member
\circled{85} of the fuselage by two 6mm bolts
\textit{(fig. \ref{fig:eightytwo})}.

\begin{figure}
  \includegraphics[width=\linewidth]{fig-82.jpg}
  \caption{Rear-wing pivot}
  \label{fig:eightytwo}
\end{figure}

The fittings \textbf{C} are fastened with 5mm bolts under the fuselage
where the 20$\times$20 members are reinforced with hardwood
\circled{37}.  Connect them with strip of sheet metal 1mm$\times$20mm.

Attach the rear wing to its pivot, using a 7mm bolt through the tube
\circled{195}.  Temporarily fasten the 1.5mm wire from \textbf{B} to
\textbf{C} so the wing is held horizontal, with its tips equidistant
from the front point of the fuselage as verified with string.

\section{Front wing}

The top of the cabane is held in place by the tube \circled{194},
fastened at the other end to the engine, so that a plumb-bob from the
axis of the pivot \circled{195} falls 123cm ahead of the nose of the
rear wing, with the fuselage supported so the top of the turtledeck
makes a 6\textsuperscript{o} angle from the horizontal.  (See
\textit{fig. \ref{fig:eightythree}} for a complete diagram of the
centering.)
...
\section{Bracing wires}

At this point you may determine the precise lengths \textbf{AB},
\textbf{BC}, \textbf{CD}, and \textbf{DE} between the holes in the
fittings, so you can prepare the final guy wires as shown in detail
\circled{370}.

Everywhere there are 5mm rods, 5mm cables.  It is clearly
unnecessarily solid in some respects, but hardly heavier --- and it's
probably best that the Pou-du-Ciel not fall apart in the air!

When the fitting \textbf{B} is removed, prevent the wing from falling
by placing a support under the opposite side so that the wires on that
side are tautened.

Install the eyelets as at \circled{371}.

Stop-wire the turnbuckles with a 1mm wire \circled{373} wound 5 times
at each end after passing it through the eyes and clevises.  The axes
of tensioners and links will be immobilized by pins \circled{374} of
1mm steel wire.

Before cutting the cables, tie in strong wire on both sides of the
cut, about 6 or 7mm.  Cut with a blow from a hammer on a cold chisel,
holding the cable against a sheet of steel plate.

Bind all cable ends that extend beyond 

%% Ligaturez tous les bouts de câbles dépassant des serre-câbles (de 10 cm) afin de ne pas vous piquer les doigts.  Laissez les câbles détortillés aux gens sales et désordonnés.
